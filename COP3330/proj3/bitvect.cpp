...
namespace fsu
{
    ...
    void BitVector::Expand (size_t newsize) // increase number of bits, retaining current values
    {
     ...
     //  return;
        ...
        //locally declared pointer
        unsigned char * newByteArray;
        
        //initialize newByteArraySize
        size_t newByteArraySize;
        
        //if the new size is bigger than current size
        if (newsize > Size())
        {
            ...
            //set byteArraySize to newByteArraySize
            byteArraySize_ = newByteArraySize;
 
            //deallocate byteArray_
            delete[] byteArray_;
            
            //point byteArray_ to newly allocated byte array
            byteArray_ = newByteArray;
        }
        
        //if new proposed size is less than current
